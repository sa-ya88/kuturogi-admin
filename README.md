...
- **お知らせ** — 専用画面でタイトル・本文・掲載日の作成・編集・削除。保存すると顧客サイトのトップとニュース一覧へ反映される（共有テーブル `news`）。管理者はデモモード中も削除できる。一般スタッフは閲覧のみ
- **売上** — 予約確定時に自動計上
...
- **デモモード** — ログイン案内、客室・プランなどのマスタ削除の禁止（お知らせは削除可）、数時間ごとのデータ初期化（`DEMO_MODE=true`）
...
- 客室・プラン・在庫・予約・お知らせ（`news`）は共通テーブル。管理画面で投稿したお知らせが顧客サイトに出る
