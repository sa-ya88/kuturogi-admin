...
class PlanImageStorageService
{
    ...
    {
        $directory = storage_path('app/plan-image-tmp');
    ...
    public function filename(int $planId, int $sequence, string $extension): string
    {
        return sprintf('plan_%d_%d.%s', $planId, $sequence, $this->normalizeExtension($extension));
    }
    ...
    public function syncImages(int $planId, array $items, array $previousImages = []): array
    {
        ...
        foreach ($items as $index => $item) {
            ...
            $finalName = $this->filename($planId, $sequence, $extension);
        ...
        $this->deletePlanImageFiles($planId, $finalNames);
    ...
    private function deletePlanImageFiles(int $planId, array $keepFilenames = []): void
    {
        ...
        foreach (File::glob($directory.'/plan_'.$planId.'_*') ?: [] as $path) {
            $basename = basename($path);
